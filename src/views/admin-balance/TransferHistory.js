...
import { getTableData, returnPaginationRange, getRangeTotalMoney } from '../../global'
import { IoSearchOutline } from 'react-icons/io5'
import { DataNotFound } from '../../components/DataNotFound'
import TransferHistoryDB from '../../database/TransferHistoryDB'
...
const TransferHistory = () => {
  ...
  //   axios({
  //     method: 'GET',
  //     url: `${config.proxy}/api/v1/admin/transaction-history?type=TRANSFER_MONEY${getRequestParams(filterInfo)}`,
  //     headers: {
  ...
  const handleSearch = () => {
    const res = searchByKeyword(TransferHistoryDB, keyword)
  ...
  useEffect(() => {
    const res = getRequestData(TransferHistoryDB, getRequestParams(filterInfo))
    setPageData(getTableData(res, page, limit))
    setPaginate(returnPaginationRange(Math.ceil(res.length / limit), page, limit, 1))
    setTotalPage(Math.ceil(res.length / limit))
    setData(res)
      ...
      <CTable align="middle" className="mb-0 border" hover responsive bordered>
        <CTableHead className="text-nowrap">
          <CTableRow>
            ...
            >
              <CIcon className="mx-1" icon={cilMoney} />
              ID
            </CTableHeaderCell>
        ...
        <CTableBody>
          ...
          {data && Object.keys(data).length === 0 && <DataNotFound colSpan={6} />}
        </CTableBody>
      </CTable>
      {!data && (
