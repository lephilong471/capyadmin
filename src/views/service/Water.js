...
import WaterDB from '../../database/WaterDB'
...
const Water = () => {
  ...
  const handleSearch = () => {
    setData(null)
    setPageData(null)
    const res = searchByKeyword(WaterDB, keyword)
  ...
  //   axios({
  //     method: 'GET',
  //     url: `${config.proxy}/api/v1/admin/transaction-history?type=USE_SERVICE&description=Payment water bill${getRequestParams(filterInfo)}`,
  //     headers: {
  ...
  useEffect(() => {
    const res = getRequestData(WaterDB, getRequestParams(filterInfo))
    setPageData(getTableData(res, page, limit))
    setPaginate(returnPaginationRange(Math.ceil(res.length / limit), page, limit, 1))
    setTotalPage(Math.ceil(res.length / limit))
    setData(res)
      ...
      <CTable align="middle" className="mb-0 border" hover responsive bordered>
        <CTableHead className="text-nowrap">
          <CTableRow>
            ...
            >
              <IoIosWater className="fs-5 text-info" />
              ID
            </CTableHeaderCell>
