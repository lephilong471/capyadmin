...
import ElectricityDB from '../../database/EletricityDB'
...
const Electricity = () => {
  ...
  //   axios({
  //     method: 'GET',
  //     url: `${config.proxy}/api/v1/admin/transaction-history?type=USE_SERVICE&description=Payment electric bill${getRequestParams(filterInfo)}`,
  //     headers: {
  //       Authorization: 'Bearer ' + adminToken,
  //     },
  //   }).then(function (res) {
  //     if (res.status === 200) {
  //       setPageData(getTableData(ElectricityDB, page, limit))
  //       setPaginate(returnPaginationRange(Math.ceil(ElectricityDB.length / limit), page, limit, 1))
  //       setTotalPage(Math.ceil(ElectricityDB.length / limit))
  //       setData(ElectricityDB)
  //     }
  //   })
  // }, [filterInfo])
  ...
  const handleSearch = () => {
    setData(null)
    setPageData(null)
    const res = searchByKeyword(ElectricityDB, keyword)
  ...
  useEffect(() => {
    const res = getRequestData(ElectricityDB, getRequestParams(filterInfo))
    setPageData(getTableData(res, page, limit))
    setPaginate(returnPaginationRange(Math.ceil(res.length / limit), page, limit, 1))
    setTotalPage(Math.ceil(res.length / limit))
    setData(res)
      ...
      <CTable align="middle" className="mb-0 border" hover responsive bordered>
        <CTableHead className="text-nowrap">
          <CTableRow>
            ...
            >
              <MdElectricBolt className="fs-5 text-danger" />
              ID
            </CTableHeaderCell>
